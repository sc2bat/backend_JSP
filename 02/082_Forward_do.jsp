...
%>
		<script type="text/javascript">
			alert("20세 미만으로 입장 불가");
			history.go(-1);
		</script>
		<% 
		}else{
		// request 객체에는 Attribute 라는 저장소가 있습니다
// 		전달할 데이터를 Atrribute 에다가 각 자료의 이름을 같이 저장하고, forward(이동)하면
...
// 		저장 메소드 : setAttribute() 추출 메소드 : getAttribute()
		...
		// 보통 파라미터를 통해서 정보를 전달하고 request.getParameter() 를 상요하여 전달된 값을
		// 추출하여 상요하지만, 위의 RequestDispatcher 는 전달인수 대신 request 내부의
		// Attribute 를 사용하여 전달인자를 저장하고, 수명을 다할지 모를 이전 페이지의 request 와 response 를 
		// 전달인수로 전달하여 forward 라는 명령으로 페이지를 이동한 다음... 이동한 페이지에서 Attribute 를 사용하게 합니다.
