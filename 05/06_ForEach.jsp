...
<!-- 저장되었던 배열을 JSP 문법으로 꺼내서 출력한다면 -->
	<%
// 		String []mlist = request.getAttribute("movieList");
	// 	그냥 저장하면 에러발생
...
<%-- <c:forEach items="${mlist }"> --%><!-- jsp 로 사용한다면 -->
	<c:forEach items="${movieList}" var="movie"><!-- jstl 사용 -->
		${movie } <br>
	</c:forEach>
<!-- items : 반복실행에 이용할 리스트 또는 배열을 지정 
	  movie : 배열 또는 리스트의 값을 한번에 하나씩 저장할 때 사용할 변수 -->
</body>
</html>